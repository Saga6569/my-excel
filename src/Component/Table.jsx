...
const Table= () => {
  ...
  const creatingCoordinatesDigits = (a) => { /// создает числовой массив длинной получаемого числа  
    const result = [];
    for (let i = 1; i <= a; i++) {
      result[i - 1] = i;
    }
  ...
  const handleChange = (option) => (e) => { // это  событие  перезаписывает данные в ячейку
    console.log('123')
  ...
  const handleLooc = (option) => (e) => {  // событие двойного клика которая блокирует ячейку для изменения 
    const {iLetter, keyNumber} = option;
    const newTable = table.map((el) => {
      if (el.lineNumber === keyNumber) {
       el.cells.map((cell) => {
